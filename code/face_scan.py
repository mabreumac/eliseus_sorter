...
def effective_scan_workers(requested: int | None = None) -> int:
    """Return process count for scanning (1 = sequential)."""
    workers = SCAN_WORKERS if requested is None else requested
    if workers <= 1:
        return 1
    cpu = os.cpu_count() or 4
    return max(1, min(workers, max(cpu * 2, 8), 32))
...
def _scan_image_worker(
    args: tuple[str, str, float, float],
) -> tuple[str, list[tuple[int, int, list[float]]], Optional[str]]:
    path_str, mode_value, min_det_score, min_area_ratio = args
    face_filter = FaceFilterParams(min_det_score=min_det_score, min_area_ratio=min_area_ratio)
    encoding = encode_faces_from_path(
        Path(path_str),
        GroupPhotoMode(mode_value),
...
def scan_all_images(
    ...
    *,
    ...
    filter_tuple = (filt.min_det_score, filt.min_area_ratio)
    ...
    payloads = [
        (str(path.resolve()), mode_value, filter_tuple[0], filter_tuple[1])
