...
mkdir -p "${LOG_DIR}"
mkdir -p "${SUPPORT}/data/ground_truth" \
         "${SUPPORT}/data/test_subset" \
         "${SUPPORT}/data/group_photos" \
         "${SUPPORT}/data/output" \
         "${SUPPORT}/data/output/sorted_students"
...
if [[ ! -x "${VENV}/bin/python" ]]; then
  ...
  /bin/bash "${RESOURCES}/scripts/install.sh" >> "${INSTALL_LOG}" 2>&1 || {
    /usr/bin/osascript -e "display alert \"Installation failed\" message \"Open ${INSTALL_LOG} for details.\" as critical"
    exit 1
  }
fi

if ! "${VENV}/bin/python" "${RESOURCES}/scripts/verify_install.py" >> "${APP_LOG}" 2>&1; then
